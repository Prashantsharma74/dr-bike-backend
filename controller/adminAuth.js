...
async function subadminsignup(req, res) {
    try{
        ...
        const roleCode = getRoleCode(role);   // Get role prefix
        const randomSuffix = generateRandomSuffix();  // e.g., AB56ER
        const employeeId = `MrBike_${roleCode}_${randomSuffix}`;


        // if (!image) {
        //   return res.status(400).json({ msg: "Please enter profile pic" });
        // }
      ...
      console.error("Signup Error:", error); // log to server console
      return res.status(500).json({
        status: 500,
        message: 'Operation was not successful',
